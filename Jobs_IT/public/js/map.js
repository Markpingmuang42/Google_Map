...
function initMap() {
    ...
    $.getJSON("mapData", function (markers) {
        markers = markers.filter((item) => user_input == 'all' || item[4] == user_input)
        for (let i = 0; i < markers.length; i++) {
            let marker = new google.maps.Marker({
                position: new google.maps.LatLng(markers[i][2], markers[i][3]),
                icon: (markers[i][4] == 'fulltime') ? '../image/bluepoint03.png' : '../image/greenpoint02.png',
                map: map,
                optimized: false,
                title: markers[i][1]
            });
            ...
            marker.addListener('click', () => {
                ...
                document.getElementById("nameCompany").innerHTML =markers[i][1]
